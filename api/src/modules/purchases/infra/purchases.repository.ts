...
export class PurchasesRepository implements IPurchasesRepository {
  ...
  async createPurchase(
    ...
    return this.db.transaction(async tx => {
      ...
      for (const member of membersInput) {
        ...
        // Batch-friendly installment creation: collect all months first
  ...
  async getPurchasesTrend(userId: string, year: number): Promise<ChartDataRow[]> {
    return this.db
      .select({
        cardName: cards.name,
  ...
  async getPurchasesYears(userId: string): Promise<number[]> {
  ...
  async getMonthTotalAmount(userId: string, month: number, year: number, today: number): Promise<number> {
    const [result] = await this.db
      ...
      .innerJoin(cards, and(eq(purchases.cardId, cards.id), eq(cards.ownerUserId, userId)))
      .where(and(eq(invoices.month, month), eq(invoices.year, year), gt(cards.dueDay, today)))
    return result?.total ?? 0
  }
