...
import type { DeleteMemberUseCase } from './delete-member.use-case'
...
export class DeleteMemberController {
  ...
  async handle(request: FastifyRequest<{ Params: { memberId: string } }>, reply: FastifyReply) {
    try {
      ...
      return reply.status(200).send({ message: 'Membro excluído com sucesso. Histórico de despesas preservado.' })
    } catch (err) {
